...
# === DATASET PATH ===
DATASET_PATH = "/Users/ritesh/Downloads/isl-project(college)/Dataset/numbers"

# === LOAD DATA SAFELY (skip hidden files like .DS_Store) ===
number_classes = [n for n in os.listdir(DATASET_PATH) 
                  if not n.startswith('.') and os.path.isdir(os.path.join(DATASET_PATH, n))]
number_classes.sort()

# === LOAD IMAGES DICTIONARY ===
number_images = {}
for num in number_classes:
...
def load_new_question():
    global current_number, question_count
    ...
    current_number = random.choice(list(number_images.keys()))
    
    # Load a random image for that number
    img_path = random.choice(number_images[current_number])
    ...
    # === Generate 4 options ===
    options = [current_number]
    others = random.sample([n for n in number_images.keys() if n != current_number], 3)
    options.extend(others)
    random.shuffle(options)
    
    # === Update buttons ===
    for i in range(4):
        buttons[i].config(text=options[i], command=lambda n=options[i]: check_answer(n))

def check_answer(selected_number):
    global score
...
# === CREATE 4 OPTION BUTTONS (2x2 grid) ===
buttons = []
for i in range(4):
